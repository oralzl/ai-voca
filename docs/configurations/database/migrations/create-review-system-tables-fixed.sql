...
-- user_word_state 策略
DROP POLICY IF EXISTS "Users can manage their own word states" ON public.user_word_state;
CREATE POLICY "Users can manage their own word states" ON public.user_word_state
  FOR ALL USING (auth.uid() = user_id);

-- review_events 策略
DROP POLICY IF EXISTS "Users can manage their own review events" ON public.review_events;
CREATE POLICY "Users can manage their own review events" ON public.review_events
  FOR ALL USING (auth.uid() = user_id);

-- user_review_prefs 策略
DROP POLICY IF EXISTS "Users can manage their own review preferences" ON public.user_review_prefs;
...
DROP TRIGGER IF EXISTS update_user_word_state_updated_at ON public.user_word_state;
CREATE TRIGGER update_user_word_state_updated_at
  BEFORE UPDATE ON public.user_word_state
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_updated_at();

DROP TRIGGER IF EXISTS update_user_review_prefs_updated_at ON public.user_review_prefs;
