...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'word_queries' 
        AND policyname = 'Users can manage their own queries'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'user_query_limits' 
        AND policyname = 'Users can manage their own limits'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers 
        WHERE event_object_table = 'user_query_limits' 
        AND trigger_name = 'update_query_limits_updated_at'
    ) THEN
