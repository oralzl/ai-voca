...
我们现在使用**三层环境架构**确保安全开发：

```
本地开发 → 预览环境 → 生产环境
 (localhost)   (测试数据库)   (生产数据库)
```
...
**自动触发**：
- Vercel自动创建预览链接：`https://ai-voca-git-feature-new-feature-xxx.vercel.app`
- 使用独立的测试数据库，完全隔离生产数据
- 页面顶部显示"🧪 测试环境"横幅
...
**自动触发**：
- Vercel自动部署到生产环境：https://ai-voca-frontend.vercel.app
- 使用生产数据库和真实用户数据

### 🗂️ 环境配置

#### 预览环境配置文件
- **数据库脚本**：`docs/configurations/database/test-database-setup.sql`
- **配置指南**：`docs/configurations/vercel/preview-environment-setup.md`
- **验证清单**：`docs/configurations/vercel/preview-verification-checklist.md`

#### 环境变量分离
| 环境 | 数据库 | 配置范围 |
|------|--------|----------|
| **Production** (main分支) | 生产Supabase项目 | 真实用户数据 |
| **Preview** (其他分支) | 测试Supabase项目 | 测试数据，无限制查询 |
| **Local** | 连接生产或测试 | 本地开发 |
...
### 预览环境使用
- **新功能开发**：始终创建分支，通过预览环境测试后再合并
- **后端API修改**：必须在预览环境验证，确保不影响生产用户
- **数据库修改**：先在测试数据库验证，再应用到生产环境
- **环境识别**：通过页面顶部的"🧪 测试环境"横幅确认当前环境
